...
import { ChangePasswordInput, LoginInput, ResetPasswordInput, SignUpInput } from 'types/auth.type';
...
import { toggleIsAuthenticatedSlice } from '../reducers/settings.reducer';

export const SIGNUP_PROPERTIES = new Set(['firstName', 'lastName', 'email', 'password', 'username']);

interface SignUpI extends Partial<SignUpInput> {
  username: string;
}
/**
 * create user account
 ...
 * @returns
 */
export const signUp = (values: SignUpInput): any => {
 ...
 */
export const loginSuccess = (redirectToHome = false): any => {
  return actionWithLoader(async (dispatch: AppDispatch, getState?: () => RootState) => {
 ...
 * @returns
 */
export const login = (values: LoginInput): any => {
  return actionWithLoader(async (dispatch: AppDispatch, getState?: () => RootState) => {
    const state = getState?.();
    ...
    try {
      ...
      dispatch(loginSuccess(true)); // redirect
    } catch (error) {
 ...
 * @returns
 */
export const changePassword = (values: ChangePasswordInput): any => {
