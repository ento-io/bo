...
const CheckboxField = ({ name, label, tooltip, isSwitch = false }: Props) => {
  // hooks
  const {
    control,
    formState: { errors },
    ...
    <FormControl component="fieldset" error={!!errors?.[name]}>
      <Controller
        ...
        render={({ field }: any): any => (
          <FormControlLabel
            {...field}
            label={<InputLabel label={label} tooltip={tooltip} sx={{ mb: 0 }} />}
            control={<Component onChange={field.onChange} checked={!!field.value} />}
          />
        )}
      />
