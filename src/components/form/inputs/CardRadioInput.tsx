import { Box, Stack, styled, SxProps, Theme, Typography, useTheme } from '@mui/material';
...
const CardRadioInput = ({ onChange, value, sx, cardSx, options = [] }: Props) => {
    ...
    <Stack direction="row" spacing={2} sx={sx}>
      {options.map((option: ICardRadio, index: number) => (
        <StyledCard
          ...
          {option.description && (
            <Box sx={{ mt: 1 }}>
              <Typography sx={{ fontSize: 13, color: '#78777e' }}>{option.description}</Typography>
            </Box>
          )}
        </StyledCard>
      ))}
